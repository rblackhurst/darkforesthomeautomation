// Inline-save helper + sidebar navigation for the pre-install checklist
// render view. Mirrors the backend_install.js pattern exactly, but targets
// the pre-install AJAX endpoints via window.PI_BASE_URL.
  ...
  if (resetBtn) {
    resetBtn.addEventListener("click", async () => {
      const ok = window.confirm(
        `Reset all progress for ${CUSTOMER}'s pre-install checklist?\n\n` +
        `This will uncheck every box, clear every note, and clear every ` +
        `captured value. Cannot be undone.`
      );
      if (!ok) return;
